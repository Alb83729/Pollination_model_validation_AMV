...
# code | nesting_suitability | foraging_suitability
needed_cols <- c("code", "nesting_suitability_small", "foraging_suitability_small_season_1")
stopifnot(all(needed_cols %in% names(expert_tbl)))

# Construir matrices de reclasificación (from, to, becomes)
rc_nest <- cbind(from    = expert_tbl$code,
                 to      = expert_tbl$code,
                 becomes = expert_tbl$nesting_suitability_small)

rc_flor <- cbind(from    = expert_tbl$code,
                 to      = expert_tbl$code,
                 becomes = expert_tbl$foraging_suitability_small_season_1)
